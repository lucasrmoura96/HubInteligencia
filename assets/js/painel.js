...
function setupFiltros() {
  ...
  document.querySelectorAll('.preset').forEach(b => {
    b.onclick = () => {
      ...
      const ref = new Date(STATE.data.meta.data_referencia);
...
function calcKpisFiltrados() {
  ...
  // Quando há filtro de TIPO de curso, recalcula a partir de por_curso_real
  ...
  if (dia !== 'all' && mes !== 'all' && ano !== 'all') {
    ...
    return {
      ...
      cpr: 0,                    // série diária não traz reuniões — fica zerado
      reunioes: 0,
...
function calcKpisMesAnterior() {
  ...
  const dataRef = new Date(STATE.data.meta.data_referencia);
